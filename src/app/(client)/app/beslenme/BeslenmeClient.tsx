...
export default function BeslenmeClient({ clientId, trainerId, userId, memberMeals, initialLogs, today, nutritionNote }: Props) {
  ...
  async function handleMealStatus(meal: Meal, status: 'compliant' | 'non_compliant') {
    ...
    if (existingLog && existingLog.status === status) {
      ...
      if (!error && data) {
        ...
        setSuccessMsg(status === 'compliant' ? 'Uydum' : 'Uymadım')
  ...
  // --- foto yükleme ---
  async function handlePhotoUpload(file: File, meal: Meal) {
    if (file.size > 15 * 1024 * 1024) {
      alert('Dosya 15MB\'dan küçük olmalı')
      return
    }
    ...
    return (
      <div className="space-y-6 pb-8">
        <div className="flex items-center gap-3 animate-fade-up">
          <Link href="/app" className="p-2 -ml-2 text-text-secondary hover:text-text-primary transition-colors">
      ...
      {successMsg && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-50 animate-fade-up">
          <div className={`flex items-center gap-2 text-white px-4 py-2.5 rounded-xl shadow-lg text-sm font-medium ${
            successMsg === 'Uymadım' ? 'bg-red-500' : 'bg-emerald-500'
          }`}>
            {successMsg === 'Uymadım' ? (
          ...
          {past14Days.map(({ date, compliant: comp, total: tot, hasExtra }) => {
            ...
            return (
              ...
              >
                ...
                <span className={`w-7 h-7 flex items-center justify-center rounded-full text-xs font-semibold ${getGridColor(comp, tot)}`}>
                  {dayNum}
                </span>
                {hasExtra && (

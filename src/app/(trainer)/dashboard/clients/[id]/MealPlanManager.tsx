...
export default function MealPlanManager({ clientId, trainerId, initialMeals, initialNutritionNote }: Props) {
  const [meals, setMeals] = useState<ClientMeal[]>(initialMeals)
  ...
  async function handleNutritionNoteSave() {
    setSavingNote(true)
    await supabase.from('clients').update({ nutrition_note: nutritionNote || null }).eq('id', clientId)
    setSavingNote(false)
    setNoteSaved(true)
  ...
  const handleAdd = useCallback(async () => {
    ...
    const { data, error } = await supabase
      .from('client_meals')
      .insert({ client_id: clientId, trainer_id: trainerId, name, order_num: nextOrder })
      .select()
      .single()
    ...
    if (error) {
      ...
      setMeals((prev) => [...prev, data as ClientMeal])
      setNewName('')
      setAdding(false)
    }
    setSaving(false)
  }, [newName, meals, clientId, trainerId, supabase])

  const handleDelete = useCallback(async (mealId: string, mealName: string) => {
    if (!confirm(`"${mealName}" öğününü silmek istediğinize emin misiniz?`)) return

    setDeletingId(mealId)
    const { error } = await supabase.from('client_meals').delete().eq('id', mealId)
  ...
  const handleMove = useCallback(async (mealId: string, direction: 'up' | 'down') => {
    ...
    const [res1, res2] = await Promise.all([
      supabase.from('client_meals').update({ order_num: swapMeal.order_num }).eq('id', currentMeal.id),
      supabase.from('client_meals').update({ order_num: currentMeal.order_num }).eq('id', swapMeal.id),
    ])
    ...
    <div className="rounded-xl border border-border p-4 sm:p-5 bg-surface">
      ...
      {meals.length > 0 ? (
        ...
        <p className="text-sm text-text-secondary py-2">
          Henüz öğün tanımlanmamış. Beslenme planını oluşturmak için öğün ekleyin.
        </p>
      ) : null}
      ...
      <div className="mt-4 pt-4 border-t border-border/50">
        ...
        <textarea
          ...
          placeholder="Genel beslenme notu... (danışan beslenme sayfasında görecek)"
          rows={2}

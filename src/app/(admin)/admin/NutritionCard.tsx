  ...
  return (
    <Card className="border-primary/30">
      ...
      {expanded && <div className="space-y-1 mt-3">
        {data.map((member) => {
          const pct = member.totalCount > 0 ? (member.completedCount / member.totalCount) * 100 : 0
          const isOpen = openId === member.userId
          const barColor = pct >= 100 ? 'bg-emerald-500' : pct >= 50 ? 'bg-amber-500' : pct > 0 ? 'bg-red-500' : 'bg-border'

          return (
            <div key={member.userId}>
              <div
                onClick={() => setOpenId(isOpen ? null : member.userId)}
                className="w-full flex items-center gap-3 py-2 px-2 -mx-1 rounded-lg hover:bg-surface-hover active:bg-surface-hover transition-colors cursor-pointer"
              >
                <Link
